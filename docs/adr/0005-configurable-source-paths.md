...
设置页的来源健康表始终展示当前实际生效的扫描路径（`root_path`，多个路径用「, 」连接），环境变量覆盖后这里会如实反映，不需要用户去猜路径拼接结果对不对。

## Consequences

- 环境变量只在应用启动/摄取时读取一次性生效；桌面 App 不是从终端环境启动的（比如从 Dock/Finder 打开），系统级环境变量在部分平台上可能读不到用户 shell 里 export 的值——这是已知限制，文档里需要说明「更推荐用 launchd/系统级环境变量，或者从终端启动」。
- 路径解析逻辑（`ingest::source_scan_dirs_with`）和真实的环境变量读取（`ingest::env_overrides`）分离，测试直接构造覆盖表验证拼接规则，不触碰进程级环境变量，避免并行测试互相污染。
