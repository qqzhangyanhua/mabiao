...
**决定**：摄取缓存采用“最后一次正确结果”语义。文件只有在完整读取和结构校验成功后，才替换该文件的旧消耗记录并更新缓存标记；失败文件保留旧记录并作为来源级诊断返回。每个 Source 扫描成功后，按本轮实际看到的路径对账：`ingest.rs` 调 `store::reconcile_source`，**已缓存但本轮未见的路径不再物理删除**，而是给对应 `usage_records` 打 `archived_at` 并清 `ingested_files` 指纹（见 ADR 0004）。缓存键除主文件元数据外，还包含适配器版本以及影响归一化结果的辅助输入指纹。
...
- 单文件失败不会阻断其他文件和来源；报告标记为部分成功。该来源本轮只要出现失败，就**跳过归档对账**（`files_failed > 0` 时不调 `reconcile_source`），避免把「解析失败」误判成「文件已消失」。
- 来源扫描发生目录读取错误时不得做归档对账，避免把“无法读取”误判成“文件已删除”。
