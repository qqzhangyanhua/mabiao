...
- **独立 opt-in**：刷新是独立按钮，不进入 `ingest_all` / 启动摄取 / 本机会话的 1/5/10 分钟定时器。离线时只读上次缓存，不阻塞其它来源。用户可在设置 Cursor 页打开独立自动刷新（默认关，固定 10 分钟，独立存储键），仍不进本机 KPI / 5 小时窗。**自动刷新由 webview 定时器实现**（`useCursorAccountRefresh` + `localStorage`），不是 Rust 后台任务。
