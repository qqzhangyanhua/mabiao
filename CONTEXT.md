...
用户在设置页自行登记的、按内置预设类型取数的账号级额度来源（第三方 API 中转站、聚合服务）。属于「官方额度」维度，与消耗记录、本机 5 小时/7 天估计窗、Cursor 账号用量、代码量互不相干，不进本机 token KPI。标识随机生成、带 `custom:` 前缀，与内置账号永不冲突；名称是纯展示标签，改名不改标识，额度缓存与告警去重记录跟着标识走。配置与密钥分两份文件存，密钥不进备份。取数只走内置预设类型的解析器，只打计费/余额接口。见 `docs/adr/0012-custom-quota-providers.md`。
