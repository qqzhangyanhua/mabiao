...
pub(super) fn parse_from_values(
    ...
    let parent_session_id = crate::adapters::omp::parent_session_id(path);
    let is_top_level = parent_session_id.is_none();
    ...
    if let Some(parent_id) = parent_session_id {
        ...
        details.insert("parent_id".to_string(), Value::String(parent_id));
        events.push(semantic_event(
            0,
            EventKind::SystemStatus,
            &started_at,
