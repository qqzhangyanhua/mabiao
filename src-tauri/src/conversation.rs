...
use toolbox::{
    append_message_delta, assign_event_provenance, attachment_candidates, compare_event_order,
    compare_event_timestamps, compare_optional_timestamps, compare_timestamps,
    deduplicate_message_channels, event_message, event_msg_semantic_event, first_text,
    flush_message_delta, message_event, populate_attachments, response_message,
    response_semantic_event, semantic_event, strip_message_bodies_from_details,
    strip_prompt_wrappers, text_field, truncate_title, unadapted_event, CAPABILITY_EVENTS,
...
fn model_label_from_models_json(raw: &str) -> String {
...
fn cursor_hash_models_by_session(
    conn: &Connection,
    session_ids: &[String],
...
fn apply_cursor_hash_model(session: &mut ConversationSessionRow, model: Option<&str>) {
...
fn sync_cursor_hash_models(conn: &Connection) -> Result<(), String> {
    let mut statement = conn
        .prepare(
            r#"
...
fn fill_empty_cursor_hash_model(
    conn: &Connection,
